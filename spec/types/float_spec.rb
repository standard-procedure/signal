RSpec.describe StandardProcedure::Signal::Attribute::Float do
  it "converts values to floats" do
    ["String", 123, 45.6, Time.now].each do |value|
      attribute = StandardProcedure::Signal::Attribute.float value
  ...
  it "raises a FormatError if it cannot perform the conversion" do
    [Date.today, false].each do |value|
      expect { StandardProcedure::Signal::Attribute.float(value) }.to raise_exception(StandardError)
    end
  end

  it "preserves nils" do
    attribute = StandardProcedure::Signal::Attribute.float nil
